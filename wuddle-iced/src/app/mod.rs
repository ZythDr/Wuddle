...
use crate::types::*;
use crate::theme::{self, ThemeColors, WuddleTheme, FRIZ, NOTO, LIFECRAFT};
use crate::service::{self, PlanRow, RepoRow, is_mod};
...
impl App {
            ...
            log_lines: {
                ...
                if auto != 1.0 {
                    ...
                    if let Some((w, h)) = monitor::primary_monitor_size() {
                        lines.push(LogLine { level: LogLevel::Info, text: format!("Monitor {w}x{h} detected \u{2014} auto scale {pct}%").into(), timestamp: chrono_now() });
                    }
                }
    ...
    pub fn show_toast_with_action(&mut self, message: impl Into<String>, kind: ToastKind, on_click: Message) {
        self.push_toast(message, kind, Some(on_click));
    }

    pub fn push_toast(&mut self, message: impl Into<String>, kind: ToastKind, on_click: Option<Message>) {
    ...
    pub fn update(&mut self, message: Message) -> Task<Message> {
        if let Some(task) = crate::update::misc::update(self, message.clone()) { return task; }
        if let Some(task) = crate::update::radio::update(self, message.clone()) { return task; }
        if let Some(task) = crate::update::tweaks::update(self, message.clone()) { return task; }
        if let Some(task) = crate::update::about::update(self, message.clone()) { return task; }
        if let Some(task) = crate::update::settings::update(self, message.clone()) { return task; }
        if let Some(task) = crate::update::repos::update(self, message.clone()) { return task; }
        ...
        match message {
            ...
            Message::ToggleAutoCheck(_) | Message::SetAutoCheckMinutes(_) |
            Message::ToggleDesktopNotify(_) | Message::ToggleSymlinks(_) |
            Message::ToggleXattr(_) | Message::ToggleClock12(_) |
            Message::ToggleFrizFont(_) | Message::SetUiScaleMode(_) |
            ...
            // --- Phase 3: Operations ---
            Message::SaveSettings | Message::SaveGithubToken |
            Message::SaveGithubTokenResult(_) | Message::ForgetGithubToken |
            ...
            // --- Instance settings ---
            Message::UpdateInstanceField(_) | Message::SaveInstanceSettings |
            ...
            Message::DxvkConfigSaved(result) => {
                match result {
                    Ok(()) => {
                        let path = std::path::Path::new(&self.wow_dir).join("dxvk.conf");
                        self.log(LogLevel::Info, &format!("Saved dxvk.conf \u{2192} {}", path.display()));
                        self.dialog = None;
                    }
    ...
    pub fn layer_toasts<'a>(&'a self, base: Element<'a, Message>, colors: &ThemeColors) -> Element<'a, Message> {
        let c = *colors;
    ...
    pub fn view_dialog<'a>(&'a self, dialog: &'a Dialog, colors: &ThemeColors) -> Element<'a, Message> {
        let c = *colors;
        match dialog {
                        ...
                        if !preview.files.is_empty() {
                            ...
                            for f in sorted_files.iter().take(60) {
                                ...
                                if f.is_dir {
                                    ...
                                    let folder_icon = if expanded { "\u{1f4c2}" } else { "\u{1f4c1}" }; // \u{1f4c2} / \u{1f4c1}
                                    file_rows.push(
                                        button(
                                    ...
                                    if expanded {
                                        if let Some(children) = self.add_repo_dir_contents.get(&f.path) {
                                            ...
                                            file_rows.push(
                                                row![
                                                    Space::new().width(18),
                                                    text("Loading\u{2026}").size(11).color(colors.muted),
                                                ].into()
                                            );
                                        }
                                    }
            ...
            Dialog::Changelog { title, items, loading } => {
                let body: Element<Message> = if *loading {
                    container(text(format!("Loading {}\u{2026}", title.to_lowercase())).size(13).color(colors.muted))
                        .center_x(Length::Fill)
                        .center_y(Length::Fill)
                        .width(Length::Fill)
                    ...
                    iced::widget::scrollable(
                        iced::widget::markdown::view_with(items, md_settings, &viewer)
                    )
                ...
                column![
                    row![
                        text(title).size(18).font(iced::Font { weight: iced::font::Weight::Bold, ..Default::default() }).color(colors.title),
                        Space::new().width(Length::Fill),
                        close_button(&c),
                    ].align_y(iced::Alignment::Center),
                    body,
